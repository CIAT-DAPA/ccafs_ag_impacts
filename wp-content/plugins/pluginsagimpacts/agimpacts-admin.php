...
function OpcionMenuMisOpciones() {
  add_menu_page('Opciones generales del sitio', 'ag-impacts option', 'manage_options', 'pc-admin.php', 'AdminOpcionesGenerales');

  add_submenu_page('pc-admin.php', 'Importador CSV', 'Importador','manage_options', 'pc-admin-importador.php', 'AdminImporterCSV');
}
...
function my_theme_drop_table ( $table_name = 'the_name_without_any_prefix' ){
  ...
  $the_removal_query = "DELETE FROM $table_name_prepared";
  $wpdb->query( $the_removal_query );

  $the_removal_query = "ALTER TABLE $table_name_prepared AUTO_INCREMENT = 1";
  $wpdb->query( $the_removal_query );
}
...
//	$cots = consultarCotizaciones();
  ...
  <form action='' method='POST'>
    <input id="articles" name="articles" value="1" type="hidden">
    <input type='submit' name='upload_btn' value='Clear articles'>
  </form>
  <form action='' method='POST'>
    <input id="estimates" name="estimates" value="1" type="hidden">
    <input type='submit' name='upload_btn' value='Clear estimates'>
  </form>
...
//    echo "Upload complete.<br>";
  ...
  if(isset($_POST['articles'])) {
    echo "Article table cleared";
    my_theme_drop_table("article");
  }
  if(isset($_POST['estimates'])) {
    echo "Estimate table cleared";
    my_theme_drop_table("estimate");
  }
}
